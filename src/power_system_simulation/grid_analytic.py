...
from power_grid_model.utils import json_deserialize_from_file, json_serialize
...
class GridAnalysis:
    def __init__(
        ...
        edge_vertex_id_pairs = [
            id for id in zip(dataset["transformer"]["from_node"], dataset["transformer"]["to_node"])
        ] + [id for id in zip(dataset["line"]["from_node"], dataset["line"]["to_node"])]
        status_enabled = [status for status in dataset["transformer"]["to_status"]] + [
            status for status in dataset["line"]["to_status"]
        ]
    ...
    def alternative_grid_topology(self, edge_id: int):
        ...
        if alternative_lines:
            for line_id in alternative_lines:
                ...
                assert_valid_batch_data(
                    input_data=self.input_data, update_data=update_data, calculation_type=CalculationType.power_flow
                )
