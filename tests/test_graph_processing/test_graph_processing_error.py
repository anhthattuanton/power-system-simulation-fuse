...
import pytest
from power_system_simulation.graph_processing import GraphProcessor, IDNotFoundError, IDNotUniqueError, InputLengthDoesNotMatchError, GraphCycleError, EdgeAlreadyDisabledError, GraphNotFullyConnectedError
...
def test_graph_fully_connected():
    ...
    with pytest.raises(GraphNotFullyConnectedError):
        result = GraphProcessor(vertex_ids= vertex_ids, 
                                edge_ids= edge_ids, 
...
def test_ID_not_unique():
    ...
    with pytest.raises(IDNotUniqueError):
        result = GraphProcessor(vertex_ids= vertex_ids, edge_ids= edge_ids, edge_vertex_id_pairs= edge_vertex_id, edge_enabled= edge_enabled, source_vertex_id= source_id)
